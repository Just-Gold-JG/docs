...
?> Delivery progress — dispatch, delivery, and proof of delivery — is tracked on the transaction's `deliveryInfo` object, which JustGold populates as the order is fulfilled. Retrieve it with [`GET /v1/transactions/:transactionId`](api/transactions.md). Note that `status` here is the transaction/payment status, whereas `deliveryInfo.status` is the physical delivery stage.
